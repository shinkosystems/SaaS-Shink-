...
import { 
    ...
    Code2, BarChart3, Plus, Activity, Microscope
} from 'lucide-react';
    ...
    const groups = [
        {
            ...
            items: [
                ...
                { id: 'framework-system', label: 'Framework Shinkō', icon: Microscope }
            ]
        },
        {
            title: 'Execução',
...
export const MobileDrawer: React.FC<Props> = (props) => {
    ...
    return (
        <>
            <div className="fixed top-0 left-0 right-0 h-20 lg:hidden z-[1000] glass-panel border-b border-slate-200 dark:border-white/5 flex items-center px-6 justify-between bg-white dark:bg-[#020203] pointer-events-auto">
            ...
            {props.isMobileOpen && (
                <div className="fixed inset-0 z-[2000] lg:hidden animate-in fade-in duration-300">
                    <div className="absolute inset-0 bg-black/90 backdrop-blur-md pointer-events-auto" onClick={() => props.setIsMobileOpen(false)}></div>
                    <div className="absolute inset-y-0 left-0 w-[85%] max-w-[320px] bg-white dark:bg-[#020203] border-r border-slate-200 dark:border-white/10 flex flex-col animate-in slide-in-from-left duration-500 shadow-[20px_0_50px_rgba(0,0,0,0.5)] overflow-hidden">
                        ...
                        <div className="h-24 flex items-center justify-between px-6 border-b border-slate-100 dark:border-white/5 shrink-0">
                            ...
                            <button 
                                ...
                                className="p-2.5 bg-slate-50 dark:bg-white/10 rounded-2xl active:scale-95 transition-all"
                            >
                        ...
                        {!isClient && (
                            <div className="px-6 py-6 space-y-3">
                                ...
                                >
                                    <PlusCircle className="w-4.5 h-4.5"/> Novo Projeto
                                </button>
                            </div>
                        )}
                        ...
                        <div className="p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-black/60">
                            <div className="flex items-center gap-4 mb-8">
                                <button onClick={props.onToggleTheme} className="p-3 bg-white dark:bg-white/10 rounded-2xl border border-slate-200 dark:border-white/10 text-slate-500 transition-all shadow-sm">
                                    {props.theme === 'dark' ? <Sun className="w-5 h-5"/> : <Moon className="w-5 h-5"/>}
                                </button>
                                <button 
                                    onClick={() => { props.onOpenFeedback(); props.setIsMobileOpen(false); }}
                                    className="p-3 bg-white dark:bg-white/10 rounded-2xl border border-slate-200 dark:border-white/10 text-amber-500 transition-all shadow-sm"
                                >
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl overflow-hidden border-2 border-amber-500/30 shadow-lg">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="text-sm font-black text-slate-900 dark:text-white truncate">{props.userData.name}</div>
                                    <div className="text-[9px] font-bold text-amber-600 dark:text-amber-500 uppercase tracking-widest">{props.currentPlan?.replace('plan_', '').toUpperCase() || 'FREE'}</div>
                                </div>
