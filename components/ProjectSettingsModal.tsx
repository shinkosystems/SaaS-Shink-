...
export const ProjectSettingsModal: React.FC<Props> = ({ opportunity, onClose, onUpdateProject, onDeleteProject }) => {
    ...
    // Fix: Using any for deleteTimerRef to avoid NodeJS namespace issues in browser environments
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-10 pb-10 space-y-10">
                    ...
                    {activeTab === 'project' && (
                        <div className="space-y-8 animate-in slide-in-from-left-4 duration-700">
                            ...
                            {/* RDE COCKPIT */}
                            <div className="p-10 bg-slate-950 rounded-[3rem] border border-amber-500/10 space-y-10 relative overflow-hidden group/rde shadow-2xl">
                                <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover/rde:scale-110 transition-transform duration-1000">
                                    <Gauge className="w-40 h-40 text-amber-500"/>
                                </div>
                                <h4 className="text-[11px] font-black text-amber-500 uppercase tracking-[0.4em] flex items-center gap-3 relative z-10">
                                    <Zap className="w-5 h-5 fill-amber-500 shadow-glow-amber"/> Engenharia de Sensibilidade RDE
                                </h4>
                                <div className="space-y-10 relative z-10">
                                    {[
                                        { key: 'velocity', label: 'Rapidez de Entrega', icon: Zap },
                                        { key: 'viability', label: 'Viabilidade Técnica', icon: Target },
                                        { key: 'revenue', label: 'Peso de Retorno', icon: DollarSign }
                                    ].map(item => (
                                        <div key={item.key} className="space-y-4">
                                            <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                                                <span className="flex items-center gap-2 group-hover/rde:text-slate-300 transition-colors">
                                                    <item.icon className="w-4 h-4 text-amber-500/50"/>
                                                    {item.label}
                                                </span>
                                                <span className="text-amber-500 text-xl font-black drop-shadow-glow">{(projectForm as any)[item.key]}</span>
                                            </div>
                                            <div className="relative group/track">
                                                <input 
                                                    type="range" min="1" max="5" step="1"
                                                    value={(projectForm as any)[item.key]}
                                                    onChange={e => setProjectForm({...projectForm, [item.key]: parseInt(e.target.value)})}
                                                    className="w-full h-1.5 bg-white/5 rounded-full appearance-none cursor-pointer accent-amber-500 z-10 relative"
                                                />
                                                <div 
                                                    className="absolute top-0 left-0 h-1.5 bg-amber-500/30 blur-sm rounded-full pointer-events-none transition-all"
                                                    style={{ width: `${((projectForm as any)[item.key] - 1) * 25}%` }}
                                                />
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <div className="border-t border-slate-100 dark:border-white/5 pt-8">
                                    <div className="p-6 bg-red-500/5 border border-red-500/10 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-500 font-bold uppercase mt-2">Segure para destruir o ativo permanentemente</p>
                                        </div>
                                        <button 
                        ...
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
                            {loadingClient ? (
                                <div className="py-20 flex flex-col items-center gap-6 text-slate-400">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-[0.4em]">Auditando Stakeholder...</span>
                                </div>
                            ) : clientData ? (
                                <>
                                    <BentoCard className="flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
                                        <div className="w-28 h-28 rounded-[2.5rem] bg-slate-100 dark:bg-white/5 border-4 border-white dark:border-white/5 overflow-hidden flex items-center justify-center shadow-2xl">
                                            {clientData.logo_url ? <img src={clientData.logo_url} className="w-full h-full object-cover"/> : <Building2 className="w-14 h-14 text-slate-300"/>}
                                        </div>
                                        <div>
                                            <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">{clientData.nome}</h3>
                                            <div className="flex wrap gap-2 mt-4">
                                                <span className="px-3 py-1 bg-slate-100 dark:bg-white/5 rounded-lg text-[9px] font-black text-slate-500 uppercase tracking-widest border border-slate-200 dark:border-white/10">ID: {clientData.id}</span>
                                                <span className="px-3 py-1 bg-emerald-500/10 rounded-lg text-[9px] font-black text-emerald-500 uppercase tracking-widest border border-emerald-500/20">Stakeholder Verificado</span>
                                            </div>
                                        </div>
                                    </BentoCard>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <BentoCard>
                                            <InputLabel icon={User}>Identificação</InputLabel>
                                            <input 
                                                value={clientForm.nome || ''} 
                                                onChange={e => setClientForm({...clientForm, name: e.target.value})}
                                                className="w-full bg-transparent text-sm font-bold outline-none text-slate-900 dark:text-white border-b border-transparent focus:border-amber-500 pb-1 transition-all"
                                                placeholder="Nome Completo..."
                                            />
                                        </BentoCard>
                                        <BentoCard>
                                            <InputLabel icon={Mail}>Comunicação (Email)</InputLabel>
                                            <input 
                                        ...
                                        <BentoCard>
                                            <InputLabel icon={Phone}>Contato Direto</InputLabel>
                                            <input 
                                        ...
                                        <BentoCard>
                                            <InputLabel icon={Hash}>Registro Fiscal (CNPJ/CPF)</InputLabel>
                                            <input 
                                    ...
                                    <BentoCard>
                                        <InputLabel icon={MapPin}>Sede Operacional / Endereço</InputLabel>
                                        <textarea 
                                            value={clientForm.endereco || ''} 
                                            onChange={e => setClientForm({...clientForm, endereco: e.target.value})}
                                            className="w-full h-24 bg-transparent text-sm font-medium outline-none text-slate-900 dark:text-white resize-none leading-relaxed"
                                            placeholder="Descreva a localização técnica..."
                                        />
                                    </BentoCard>
                                    ...
                                    <div className="pt-6">
                                        <button 
                                            ...
                                            disabled={isSaving}
                                            className="w-full py-6 bg-slate-900 dark:bg-white text-white dark:text-black rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] shadow-xl flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-30"
                                        >
                                ...
                                <div className="py-32 flex flex-col items-center justify-center text-center gap-8 group/empty">
                                    <div className="w-24 h-24 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-white/10 flex items-center justify-center group-hover/empty:border-amber-500/30 transition-all">
                                        <ShieldAlert className="w-10 h-10 text-slate-300 dark:text-white/20"/>
                                    </div>
                                    <div className="space-y-3">
                                        <h4 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Vínculo Indefinido.</h4>
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest max-w-xs mx-auto leading-relaxed">Este ativo não está associado a um stakeholder externo. Cadastre um novo parceiro na tela de "Stakeholders" para habilitar a visibilidade compartilhada.</p>
                                    </div>
                                </div>
                            )}
                        </div>
