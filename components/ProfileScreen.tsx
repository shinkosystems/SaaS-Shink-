...
export const ProfileScreen: React.FC = () => {
        ...
        if (user) {
          ...
          if (activeSub) {
             ...
             if (matchedPlan) setCurrentPlan(matchedPlan.id);
              ...
              const lastPaid = history.find(p => p.status === 'RECEIVED' || p.status === 'CONFIRMED');
              if (lastPaid) {
  ...
  const handleExecutePayment = async (method: 'PIX' | 'CREDIT_CARD') => {
      ...
      try {
          ...
          if (method === 'PIX' && res.qrCode) {
              setPixData({ qrCode: res.qrCode, copyPaste: res.copyPaste || '' });
              
              const pollTimer = setInterval(async () => {
                  const status = await checkPaymentStatus(res.payment.id);
              ...
              if (res.payment.status === 'RECEIVED' || res.payment.status === 'CONFIRMED' || res.payment.status === 'PENDING') {
                  setPaymentSuccess(true);
                  setCurrentPlan(showCheckout.id);
