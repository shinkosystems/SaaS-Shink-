        ...
        <div className="h-full flex flex-col p-6 md:p-10 overflow-y-auto custom-scrollbar animate-in fade-in duration-500">
            ...
            <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                        <Activity className="w-8 h-8 text-purple-500" /> Indicadores de Produto (Dados Reais)
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-3xl">
                        Métricas calculadas diretamente da base de usuários e eventos do sistema.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <StatCard 
                    ...
                    subtext={`${metrics.dau} usuários ativos hoje / ${metrics.mau} mensais`}
                    icon={Zap}
                ...
                <StatCard 
                    ...
                    subtext="Baseado em respostas reais"
                    icon={Heart}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                ...
                <div className="lg:col-span-2 glass-panel p-6 rounded-2xl border border-white/10 bg-white/50 dark:bg-slate-800/60">
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                        <MousePointer2 className="w-5 h-5 text-slate-400"/> Volume de Criação (Proxy de Uso)
                    </h3>
                        ...
                        {metrics.featureEngagement.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={metrics.featureEngagement} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                                    ...
                                    />
                                    <Bar dataKey="count" name="Registros Criados" fill="#8b5cf6" radius={[0, 4, 4, 0]} barSize={20} />
                                </BarChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="flex items-center justify-center h-full text-slate-500">Sem dados de uso no período.</div>
                        )}
                    </div>
                </div>
            </div>
